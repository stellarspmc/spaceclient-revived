...
public final class ItemStack {
    ...
    public List<String> getTooltip(EntityPlayer playerIn, boolean advanced) {
        ...
        if (this.hasDisplayName()) s = EnumChatFormatting.ITALIC + s;
        ...
        if (advanced) {
            ...
            if (this.getHasSubtypes()) s = s + String.format("#%04d/%d%s", i, this.itemDamage, s1);
            else s += String.format("#%04d%s", i, s1);
        } else if (!this.hasDisplayName() && this.item == Items.filled_map) s = s + " #" + this.itemDamage;
        ...
        if ((i1 & 32) == 0) this.item.addInformation(this, playerIn, list, advanced);
        ...
        if (this.hasTagCompound() && this.stackTagCompound.hasKey("CanDestroy", 9) && (i1 & 8) == 0) {
            ...
            if (nbttaglist2.tagCount() > 0) {
                ...
                for (int k1 = 0; k1 < nbttaglist2.tagCount(); ++k1) {
                    ...
                    if (block != null) list.add(EnumChatFormatting.DARK_GRAY + block.getLocalizedName());
                    else list.add(EnumChatFormatting.DARK_GRAY + "missingno");
        ...
        if (this.hasTagCompound() && this.stackTagCompound.hasKey("CanPlaceOn", 9) && (i1 & 16) == 0) {
            ...
            if (nbttaglist3.tagCount() > 0) {
                ...
                for (int l1 = 0; l1 < nbttaglist3.tagCount(); ++l1) {
                    ...
                    if (block1 != null) list.add(EnumChatFormatting.DARK_GRAY + block1.getLocalizedName());
                    else list.add(EnumChatFormatting.DARK_GRAY + "missingno");
                }
            }
        }
        ...
        if (this.getItem() instanceof ItemEnchantedBook) {
            final ItemEnchantedBook item = (ItemEnchantedBook) this.getItem();
            final List<Enchantment> enchants = EnchantmentTooltips.getEnchantments(item, this);

            for (final Enchantment enchant : enchants) {
                list.add(I18n.format("tooltip.enchdesc.name") + ": " + I18n.format(enchant.getName()));
                list.add(EnchantmentTooltips.getDescription(enchant));
            }
        }
