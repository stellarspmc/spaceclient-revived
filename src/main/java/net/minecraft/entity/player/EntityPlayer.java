...
import fun.spmc.capes.CapeHolder;
import fun.spmc.capes.StickSimulation;
...
public abstract class EntityPlayer extends EntityLivingBase implements PlayerSettings, CapeHolder {
    /**
    ...
    public static BlockPos getBedSpawnLocation(World worldIn, BlockPos bedLocation, boolean forceSpawn) {
        ...
        if (block != Blocks.bed) if (!forceSpawn) return null;
        else {
        ...
        else return BlockBed.getSafeExitLocation(worldIn, bedLocation, 0);
    }
    ...
    public void onUpdate() {
        ...
        if (this.isSpectator()) this.onGround = false;

        if (this.itemInUse != null) {
            ItemStack itemstack = this.inventory.getCurrentItem();

            if (itemstack == this.itemInUse) {
                if (this.itemInUseCount <= 25 && this.itemInUseCount % 4 == 0) this.updateItemUse(itemstack, 5);

                if (--this.itemInUseCount == 0 && !this.worldObj.isRemote) this.onItemUseFinish();
            } else this.clearItemInUse();
        }
        ...
        if (this.isPlayerSleeping()) {
            ...
            if (!this.worldObj.isRemote) if (!this.isInBed()) this.wakeUpPlayer(true, true, false);
        ...
        if (d5 > d2) this.prevChasingPosX = this.chasingPosX = this.posX;

        if (d1 > d2) this.prevChasingPosZ = this.chasingPosZ = this.posZ;

        if (d0 > d2) this.prevChasingPosY = this.chasingPosY = this.posY;
        ...
        if (d3 != this.posX || d4 != this.posZ) this.setPosition(d3, this.posY, d4);
    ...
    protected void onItemUseFinish() {
        if (this.itemInUse != null) {
            ...
            if (itemstack != this.itemInUse || itemstack != null && itemstack.stackSize != i) {
                ...
                if (itemstack.stackSize == 0) this.inventory.mainInventory[this.inventory.currentItem] = null;
            }
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 9) this.onItemUseFinish();
        else if (id == 23) this.hasReducedDebug = false;
        else if (id == 22) this.hasReducedDebug = true;
    ...
    public void onLivingUpdate() {
        ...
        if (this.worldObj.getDifficulty() == EnumDifficulty.PEACEFUL && this.worldObj.getGameRules().getBoolean("naturalRegeneration")) {
            if (this.getHealth() < this.getMaxHealth() && this.ticksExisted % 20 == 0) this.heal(1.0F);
        ...
        if (!this.worldObj.isRemote) iattributeinstance.setBaseValue(this.capabilities.getWalkSpeed());
        ...
        if (this.getHealth() > 0.0F && !this.isSpectator()) {
            ...
            else axisalignedbb = this.getEntityBoundingBox().expand(1.0D, 0.5D, 1.0D);

            List<Entity> list = this.worldObj.getEntitiesWithinAABBExcludingEntity(this, axisalignedbb);

            for (Entity entity : list) if (!entity.isDead) this.collideWithPlayer(entity);
        }
    }
    ...
    public void onDeath(DamageSource cause) {
        ...
        if (this.getName().equals("Notch")) this.dropItem(new ItemStack(Items.apple, 1), true, false);
    ...
    public void addToPlayerScore(Entity entityIn, int amount) {
        ...
        } else this.triggerAchievement(StatList.mobKillsStat);
    ...
    public EntityItem dropItem(ItemStack droppedItem, boolean dropAround, boolean traceItem) {
        if (droppedItem == null) return null;
        else if (droppedItem.stackSize == 0) return null;
        else {
            ...
            if (traceItem) this.triggerAchievement(StatList.dropStat);
    ...
    public float getToolDigEfficiency(Block p_180471_1_) {
        ...
        if (f > 1.0F) {
            ...
            if (i > 0 && itemstack != null) f += (float) (i * i + 1);
        }
        ...
        if (this.isInsideOfMaterial(Material.water) && !EnchantmentHelper.getAquaAffinityModifier(this)) f /= 5.0F;
    ...
    public void readEntityFromNBT(NBTTagCompound tagCompund) {
        ...
        if (this.xpSeed == 0) this.xpSeed = this.rand.nextInt();
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        if (this.isEntityInvulnerable(source)) return false;
        else if (this.capabilities.disableDamage && !source.canHarmInCreative()) return false;
        else {
            this.entityAge = 0;
            ...
            if (this.getHealth() <= 0.0F) return false;
                ...
                if (source.isDifficultyScaled()) switch (this.worldObj.getDifficulty()) {
                    case PEACEFUL -> amount = 0F;
                    case HARD -> amount *= 1.5F;
                    case EASY -> amount = amount / 2.0F + 1.0F;
                }
    ...
    public float getArmorVisibility() {
        ...
        for (ItemStack itemstack : this.inventory.armorInventory) if (itemstack != null) ++i;
    ...
    public boolean interactWith(Entity targetEntity) {
        if (this.isSpectator()) {
            if (targetEntity instanceof IInventory) this.displayGUIChest((IInventory) targetEntity);
            ...
            if (!targetEntity.interactFirst(this)) {
                if (itemstack != null && targetEntity instanceof EntityLivingBase) {
                    if (this.capabilities.isCreativeMode) itemstack = itemstack1;
    ...
    public void attackTargetEntityWithCurrentItem(Entity targetEntity) {
        if (targetEntity.canAttackWithItem()) if (!targetEntity.hitByEntity(this)) {
            ...
            if (f > 0.0F || f1 > 0.0F) {
                ...
                if (flag2) {
                    ...
                    if (flag) this.onCriticalHit(targetEntity);

                    if (f1 > 0.0F) this.onEnchantmentCritical(targetEntity);

                    if (f >= 18.0F) this.triggerAchievement(AchievementList.overkill);
                    ...
                    if (itemstack != null && entity instanceof EntityLivingBase) {
                        ...
                        if (itemstack.stackSize <= 0) this.destroyCurrentEquippedItem();
                    }

                    if (targetEntity instanceof EntityLivingBase) {
                        this.addStat(StatList.damageDealtStat, Math.round(f * 10.0F));

                        if (j > 0) targetEntity.setFire(j * 4);
                    }

                    this.addExhaustion(0.3F);
                } else if (flag1) targetEntity.extinguish();
            }
        }
    }
    ...
    public void setDead() {
        ...
        if (this.openContainer != null) this.openContainer.onContainerClosed(this);
    }
    ...
    public EnumStatus trySleep(BlockPos bedLocation) {
        if (!this.worldObj.isRemote) {
            if (this.isPlayerSleeping() || !this.isEntityAlive()) return EnumStatus.OTHER_PROBLEM;

            if (!this.worldObj.provider.isSurfaceWorld()) return EnumStatus.NOT_POSSIBLE_HERE;

            if (this.worldObj.isDaytime()) return EnumStatus.NOT_POSSIBLE_NOW;
            ...
            if (!list.isEmpty()) return EnumStatus.NOT_SAFE;
        }

        if (this.isRiding()) this.mountEntity(null);
        ...
        if (!this.worldObj.isRemote) this.worldObj.updateAllPlayersSleepingFlag();
    ...
    public void wakeUpPlayer(boolean immediately, boolean updateWorldFlag, boolean setSpawn) {
        ...
        if (this.playerLocation != null && iblockstate.getBlock() == Blocks.bed) {
            ...
            if (blockpos == null) blockpos = this.playerLocation.up();
        ...
        if (!this.worldObj.isRemote && updateWorldFlag) this.worldObj.updateAllPlayersSleepingFlag();

        this.sleepTimer = immediately ? 0 : 100;

        if (setSpawn) this.setSpawnPoint(this.playerLocation, false);
    }
    ...
    public void jump() {
        ...
        if (this.isSprinting()) this.addExhaustion(0.8F);
        else this.addExhaustion(0.2F);
    }
    ...
    public void addMovementStat(double p_71000_1_, double p_71000_3_, double p_71000_5_) {
        if (this.ridingEntity == null) if (this.isInsideOfMaterial(Material.water)) {
            ...
            if (k > 0) {
                ...
                if (this.isSprinting()) {
                    ...
                    if (this.isSneaking()) this.addStat(StatList.distanceCrouchedStat, k);
            ...
            if (l > 25) this.addStat(StatList.distanceFlownStat, l);
        }
    }
    ...
    private void addMountedMovementStat(double p_71015_1_, double p_71015_3_, double p_71015_5_) {
        if (this.ridingEntity != null) {
            ...
            if (i > 0) switch (this.ridingEntity) {
                ...
                case EntityBoat _ -> this.addStat(StatList.distanceByBoatStat, i);
                case EntityPig _ -> this.addStat(StatList.distanceByPigStat, i);
                case EntityHorse _ -> this.addStat(StatList.distanceByHorseStat, i);
                default -> {
                }
            }
        }
    ...
    protected void resetHeight() {
        if (!this.isSpectator()) super.resetHeight();
    }
    ...
    public void onKillEntity(EntityLivingBase entityLivingIn) {
        if (entityLivingIn instanceof IMob) this.triggerAchievement(AchievementList.killEnemy);

        EntityList.EntityEggInfo entitylist$entityegginfo = EntityList.entityEggs.get(EntityList.getEntityID(entityLivingIn));

        if (entitylist$entityegginfo != null) this.triggerAchievement(entitylist$entityegginfo.field_151512_d);
    }
    ...
    public void setInWeb() {
        if (!this.capabilities.isFlying) super.setInWeb();
    }
    ...
    public void addExhaustion(float p_71020_1_) {
        if (!this.capabilities.disableDamage) if (!this.worldObj.isRemote) this.foodStats.addExhaustion(p_71020_1_);
    }
    ...
    public boolean canPlayerEdit(BlockPos p_175151_1_, EnumFacing p_175151_2_, ItemStack p_175151_3_) {
        ...
        else if (p_175151_3_ == null) return false;
        else {
    ...
    protected int getExperiencePoints(EntityPlayer player) {
        if (this.worldObj.getGameRules().getBoolean("keepInventory")) return 0;
        else {
    ...
    public float getEyeHeight() {
        float f = this.isPlayerSleeping() ? 0.2F : 1.62F;
        if (this.isSneaking()) f -= 0.08F;
            ...
            if (i >= 0 && i < this.inventory.armorInventory.length) {
                ...
                if (itemStackIn != null && itemStackIn.getItem() != null)
                    if (itemStackIn.getItem() instanceof ItemArmor) {
                        if (EntityLiving.getArmorPosition(itemStackIn) != k) return false;
