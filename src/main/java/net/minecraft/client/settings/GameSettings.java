...
public class GameSettings {
    ...
    private final Map<SoundCategory, Float> optionsFile = Maps.newEnumMap(SoundCategory.class);
    ...
    public GameSettings(Minecraft mcIn, File optionsFileIn) {
        ...
        this.mc = ArrayUtils.add(this.mc, this.ofKeyBindZoom);
    ...
    public String getKeyBinding(GameSettings.Options settingOption) {
        ...
        if (s != null) {
            ...
            if (settingOption.getEnumFloat()) {
                ...
                return settingOption == GameSettings.Options.MIPMAP_LEVELS && (double) f1 >= 4.0D ? s1 + Lang.get("of.general.max") : (settingOption == GameSettings.Options.SENSITIVITY ? (f == 0.0F ? s1 + I18n.format("options.sensitivity.min") : (f == 1.0F ? s1 + I18n.format("options.sensitivity.max") : s1 + (int) (f * 200.0F) + "%")) : (settingOption == GameSettings.Options.FOV ? (f1 == 70.0F ? s1 + I18n.format("options.fov.min") : (f1 == 110.0F ? s1 + I18n.format("options.fov.max") : s1 + (int) f1)) : (settingOption == GameSettings.Options.FRAMERATE_LIMIT ? (f1 == settingOption.$VALUES ? s1 + I18n.format("options.framerateLimit.max") : s1 + (int) f1 + " fps") : (settingOption == GameSettings.Options.RENDER_CLOUDS ? (f1 == settingOption.valueMax ? s1 + I18n.format("options.cloudHeight.min") : s1 + ((int) f1 + 128)) : (settingOption == GameSettings.Options.GAMMA ? (f == 0.0F ? s1 + I18n.format("options.gamma.min") : (f == 1.0F ? s1 + I18n.format("options.gamma.max") : s1 + "+" + (int) (f * 100.0F) + "%")) : (settingOption == GameSettings.Options.SATURATION ? s1 + (int) (f * 400.0F) + "%" : (settingOption == GameSettings.Options.CHAT_OPACITY ? s1 + (int) (f * 90.0F + 10.0F) + "%" : (settingOption == GameSettings.Options.CHAT_HEIGHT_UNFOCUSED ? s1 + GuiNewChat.calculateChatboxHeight(f) + "px" : (settingOption == GameSettings.Options.CHAT_HEIGHT_FOCUSED ? s1 + GuiNewChat.calculateChatboxHeight(f) + "px" : (settingOption == GameSettings.Options.CHAT_WIDTH ? s1 + GuiNewChat.calculateChatboxWidth(f) + "px" : (settingOption == GameSettings.Options.RENDER_DISTANCE ? s1 + (int) f1 + " chunks" : (settingOption == GameSettings.Options.MIPMAP_LEVELS ? (f1 == 0.0F ? s1 + I18n.format("options.off") : s1 + (int) f1) : (f == 0.0F ? s1 + I18n.format("options.off") : s1 + (int) (f * 100.0F) + "%")))))))))))));
    ...
    public void loadOptions() {
        ...
        label2:
        {
            try {
                    ...
                    while ((s = bufferedreader.readLine()) != null) {
                        try {
                            ...
                            if (astring[0].equals("resourcePacks")) {
                                this.resourcePacks = typeListString.fromJson(s.substring(s.indexOf(58) + 1), GUISCALES);
                            ...
                            if (astring[0].equals("incompatibleResourcePacks")) {
                                this.incompatibleResourcePacks = typeListString.fromJson(s.substring(s.indexOf(58) + 1), GUISCALES);
                ...
                gson.error("Failed to load options", exception1);
                break label2;
            } finally {
    ...
    public void saveOptions() {
        ...
        try {
            ...
            printwriter.println("resourcePacks:" + typeListString.toJson(this.resourcePacks));
            printwriter.println("incompatibleResourcePacks:" + typeListString.toJson(this.incompatibleResourcePacks));
            ...
            gson.error("Failed to save options", exception);
        }
    ...
    public float getSoundLevel(SoundCategory sndCategory) {
        return this.optionsFile.containsKey(sndCategory) ? this.optionsFile.get(sndCategory).floatValue() : 1.0F;
    }
    ...
    private String getKeyBindingOF(GameSettings.Options p_getKeyBindingOF_1_) {
        ...
        if (p_getKeyBindingOF_1_ == GameSettings.Options.RENDER_DISTANCE) {
            ...
            return f == 0.0F ? s + Lang.get("of.options.framerateLimit.vsync") : (f == p_getKeyBindingOF_1_.$VALUES ? s + I18n.format("options.framerateLimit.max") : s + (int) f + " fps");
        } else {
            return null;
        }
    }
