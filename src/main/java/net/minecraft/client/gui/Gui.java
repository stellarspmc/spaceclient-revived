...
public class Gui {
    ...
    public static void drawBetterRect(int x, int y, int width, int height, Color color) {
        Tessellator tessellator = Tessellator.getInstance();
        WorldRenderer worldrenderer = tessellator.getWorldRenderer();
        GlStateManager.enableBlend();
        GlStateManager.disableTexture2D();
        GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
        GlStateManager.color((float) color.getRed(), (float) color.getGreen(), (float) color.getBlue(), (float) color.getAlpha() / 255f);
        worldrenderer.begin(7, DefaultVertexFormats.POSITION);
        worldrenderer.pos((x), (y + height), 0D).endVertex();
        worldrenderer.pos((x + width), (y + height), 0D).endVertex();
        worldrenderer.pos((x + width), (y), 0D).endVertex();
        worldrenderer.pos((x), (y), 0D).endVertex();
        tessellator.draw();
        GlStateManager.enableTexture2D();
        GlStateManager.disableBlend();
    }
    ...
    public void drawTexturedModalRect(float xCoord, float yCoord, int minU, int minV, int maxU, int maxV) {
        ...
        WorldRenderer worldrenderer = tessellator.getWorldRenderer();
        worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX);
        worldrenderer.pos((xCoord), (yCoord + (float) maxV), this.zLevel).tex(((float) (minU) * f), ((float) (minV + maxV) * f1)).endVertex();
        worldrenderer.pos((xCoord + (float) maxU), (yCoord + (float) maxV), this.zLevel).tex(((float) (minU + maxU) * f), ((float) (minV + maxV) * f1)).endVertex();
        worldrenderer.pos((xCoord + (float) maxU), (yCoord), this.zLevel).tex(((float) (minU + maxU) * f), ((float) (minV) * f1)).endVertex();
        worldrenderer.pos((xCoord), (yCoord), this.zLevel).tex(((float) (minU) * f), ((float) (minV) * f1)).endVertex();
        tessellator.draw();
    }
